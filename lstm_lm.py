...
class Lstm_LanguageModel(object):
    ...
    def __init__(self, is_training, num_steps, VOCAB_SIZE, HIDDEN_SIZE, NUM_LAYERS, dropout_keep_prob):
        ...
        outputs, state = tf.nn.dynamic_rnn(cell, inputs, self.sequence_length, dtype=tf.float32)

        # outputs shape
        fc_w = tf.Variable(tf.truncated_normal([HIDDEN_SIZE, VOCAB_SIZE], stddev=0.1), name='fc_w')
